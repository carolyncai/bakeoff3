...
void mousePressed()
{
  ...
  if (didMouseClick(width/2-sizeOfInputArea/2, height/2-sizeOfInputArea/2, sizeOfInputArea, sizeOfInputArea/4)) //check if click occured in letter area
  {
    ...
    if (currentLetter=='`' && currentWord.length()>0 && currentTyped.length() > 0){
         currentWord = currentWord.substring(0, currentWord.length()-1);
      }
    if(currentLetter=='`' & currentTyped.length()>0)
      {//if `, treat that as a delete command
      ...
      System.out.println(currentTyped.trim());
      System.out.println(currentPhrase.trim());
    }
